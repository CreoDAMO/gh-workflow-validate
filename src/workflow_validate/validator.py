...
class YAMLValidator:
    ...
    def validate_file(self, file_path: Path) -> ValidationResult:
        ...
        if isinstance(data, dict):
            ...
            struct['has_permissions'] = 'permissions' in data or any(isinstance(job, dict) and 'permissions' in job for job in data.get('jobs', {}).values())
            
            if 'jobs' in data:
                struct['jobs'] = sorted(data['jobs'].keys())
                struct['job_count'] = len(data['jobs'])
